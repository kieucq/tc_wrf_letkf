...
subaddh() {
	...
	local _DATE_STR="1970-01-01 $(($(date -d "${_DATE} ${_HH} UTC" "+%s") ${_OP} ${range})) sec"
	eval ${_ED_YY}=`date -d "${_DATE_STR}" "+%Y"`
	eval ${_ED_MM}=`date -d "${_DATE_STR}" "+%m"`
	eval ${_ED_DD}=`date -d "${_DATE_STR}" "+%d"`
	eval ${_ED_HH}=`date -d "${_DATE_STR}" "+%H"`
}
