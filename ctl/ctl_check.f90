...
!
       ...
       ALLOCATE(p(nx,ny,nz1,nt),w(nx,ny,nz1,nt))
       ALLOCATE(u1(nx,ny,nz,nfile),v1(nx,ny,nz,nfile),t1(nx,ny,nz,nfile),r1(nx,ny,nz,nfile))
       ALLOCATE(p1(nx,ny,nz,nfile),w1(nx,ny,nz,nfile))   
      endif
!
! reading data now
!
      ...
      p1(:,:,:,ifile) = 0.5*(p(:,:,1:nz,1)+p(:,:,2:nz1,1))
      tem          = 'W'
      tem_len      = len_trim(tem)
      status       = NF_INQ_VARID(ncid,tem(1:tem_len),pid)
      status       = NF_GET_VAR_REAL(ncid,pid,w)
      w1(:,:,:,ifile) = 0.5*(w(:,:,1:nz,1)+w(:,:,2:nz1,1))      
!
...
! printout now
!
      open(99,file='ctl_check.dat',FORM='UNFORMATTED',ACCESS='DIRECT',RECL=nx*ny*4)
      do ifile     = 1,nfile  
       do k        = 1,nz
	...
	irec       = irec + 1
       enddo
       do k        = 1,nz
        write(99,rec=irec)((w1(i,j,k,ifile),i=1,nx),j=1,ny)
         irec       = irec + 1
         enddo       
       do k        = 1,nz
	...
	irec       = irec + 1
       enddo         
       do k        = 1,nz
        write(99,rec=irec)((p1(i,j,k,ifile),i=1,nx),j=1,ny)
         irec       = irec + 1
         enddo       
      enddo 
!
