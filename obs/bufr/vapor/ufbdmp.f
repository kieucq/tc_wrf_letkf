...
c 2003-12-09  J.Whiting     removed out commentary after (q)uit
c                           limited fields being output
C
...
cjaw        PRINT*,'(<enter> for MORE, q <enter> to QUIT)'
         print '(a,$)',' more... (q)uit '
         READ(5,'(A1)') YOU
...
cjaw        IF(YOU.EQ.'q') THEN
cjaw        PRINT*
cjaw        PRINT*,'==> You have chosen to stop the dumping of this subset'
cjaw        PRINT*
cjaw           GOTO 100
cjaw        ENDIF
         IF(YOU.EQ.'q') goto 100
      else
         write(luout,*)
      ENDIF
      ND = INV (NV,LUN)
...
cjaw           WRITE(LUOUT,1) NV,TP,IT,TG,VL,IB,IS,IR,ND,JP,LK,JB
            WRITE(LUOUT,1) NV,TP,IT,TG,VL!!,IB,IS,IR,ND,JP,LK,JB
         ELSE
cjaw           WRITE(LUOUT,10) NV,TP,IT,TG,VL,IB,IS,IR,ND,JP,LK,JB
...
cccccccccIF(VL.EQ.BMISS) THEN
         ...
         ENDIF
         RJ = RJUST(VC)
cjaw         WRITE(LUOUT,2) NV,TP,IT,TG,VC,IB,IS,IR,ND,JP,LK,JB
...
cjaw1     FORMAT(I5,1X,A3,'-',I1,1X,A10,1X,G15.6,7(1X,I5))
1     FORMAT(I5,1X,A3,'-',I1,1X,A10,1X,G14.5,' ',$)                  
10    FORMAT(I5,1X,A3,'-',I1,1X,A10,1X,F15.6,7(1X,I5))
cjaw2     FORMAT(I5,1X,A3,'-',I1,1X,A10,6X,'"',A8,'"',7(1X,I5))
2     FORMAT(I5,1X,A3,'-',I1,1X,A10,1X,A10  ,' ',$)                  
