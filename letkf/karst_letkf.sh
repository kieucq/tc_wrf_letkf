...
#PBS -d .
#PBS -N tc_letkf_cimss
#PBS -o /N/u/ckieu/Karst/tc_wrf_letkf/run/log_letkf.out
#PBS -e /N/u/ckieu/Karst/tc_wrf_letkf/run/log_letkf.err
#
# [NOTE]:   
...
if [ -f $DIR_OBS/$obs_file ]; then
 ...
 if [ "$blend_obs" == "NO" ]; then
     ...
     echo " LETKF: blend_obs has to be 0 or 1... exit 1"
     exit 1
 fi
else
...
if [ "$OBSMODE" != "CONTROL" ]; then
    rm -f *~ error.status
    mpirun -n 1 ./letkf_mpi.exe >> $DIR_RUN/log.letkf
...
if [ -f ./error.status ]; then
    ...
    echo " MAIN: karst_letkf.sh has abrupt die...exit 8"
    exit 8
fi
rm -f bgd_*.dat wrfinput_d0* wrfbdy_d0* 
echo " LETKF script finished normally     "
