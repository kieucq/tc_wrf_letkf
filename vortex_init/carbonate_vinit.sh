...
set -x
. ./input.sh
./carbonate_bogus.sh ${start_time_main} > $DIR_RUN/log.bogusv
check_bogus=`grep safely $DIR_RUN/log.bogusv`
if [ "${check_bogus}" == "" ]; then
   echo " VINIT: carbonate_bogus.sh failed ... exit 1"
   exit 1
fi
cd $DIR_LETKF
rm -f wrfinput_d0* wrfbdy_d0* ./namelist*
cp  $DIR_MAN/run/input.sh ./
cp  $DIR_MAN/script/carbonate_vortda.sh ./
...
if [ "$blend_obs" == "YES" ]; then
    ...
    echo " VINIT: carbonate_bogus.sh finishes " > $DIR_RUN/status_vinit_ok
    exit 0
fi
...
./carbonate_vortda.sh ${start_time_main} ${ne} >> $DIR_RUN/log.vortda
if [ -f $DIR_LETKF/letkf.dat ]; then
    mv letkf.dat letkf_bogusv.dat
    echo " VINIT: carbonate_vortda.sh finishes safely" > $DIR_RUN/status_vinit_ok
else
    echo " VINIT: carbonate_vortda.sh step failed... exit 1"
    exit 1
fi
