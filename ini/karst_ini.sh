...
#PBS -d .
#PBS -N tc_letkf_coldstart
#PBS -o /N/u/ckieu/Karst/tc_wrf_letkf/run/log_coldstart.out
#PBS -e /N/u/ckieu/Karst/tc_wrf_letkf/run/log_coldstart.err
module unload mpich
module unload intel
...
check_ini=`ls ./bgd_*`
if [ "${check_ini}" == "" ]; then
    echo "   INI: karst_ini.sh failed... exit 6"
    exit 6
else
    echo "   INI: karst_ini.sh done" > ${DIR_RUN}/status_ini_ok
fi
echo "                                     "
