...
#
if [ $# == 6 ]; then
  echo " NAMELIST: Create namelist for DFI mode... "
  time_input=$1
  ...
  echo " NAMELIST: Please use one of these options for DFI mode: "
...
#
. ./quartz_cal_time.sh
cal_time ${data_year_nlist} ${data_month_nlist} ${data_day_nlist} ${data_hour_nlist} ${data_minute_nlist} 0 0 -30
dfi_year_bw=${E_YEAR} 
dfi_month_bw=${E_MONTH}
dfi_day_bw=${E_DAY}
dfi_hour_bw=${E_HOUR}
dfi_minute_bw=${E_MINUTE}
cal_time ${data_year_nlist} ${data_month_nlist} ${data_day_nlist} ${data_hour_nlist} ${data_minute_nlist} 0 0 30
dfi_year_fw=${E_YEAR}
dfi_month_fw=${E_MONTH}
dfi_day_fw=${E_DAY}
dfi_hour_fw=${E_HOUR}
dfi_minute_fw=${E_MINUTE}
cal_time ${data_year_nlist} ${data_month_nlist} ${data_day_nlist} ${data_hour_nlist} ${data_minute_nlist} 0 0 10
...
#
 ...
 rm -f namelist.input_dfi* 
 cat > ${DIR_RUN}/namelist.input_dfi_${mcphysics} << EOF2
 &time_control
 run_days                            = 0,
 run_hours                           = 0,
 run_minutes                         = 0,
 ...
 dfi_time_dim                        = 1000                                                     
 dfi_bckstop_year                    = ${dfi_year_bw}     
 dfi_bckstop_month                   = ${dfi_month_bw}       
 dfi_bckstop_day                     = ${dfi_day_bw}       
 dfi_bckstop_hour                    = ${dfi_hour_bw}       
 dfi_bckstop_minute                  = ${dfi_minute_bw}       
 dfi_bckstop_second                  = 00       
 dfi_fwdstop_year                    = ${dfi_year_fw}     
 dfi_fwdstop_month                   = ${dfi_month_fw}       
 dfi_fwdstop_day                     = ${dfi_day_fw}       
 dfi_fwdstop_hour                    = ${dfi_hour_fw}       
 dfi_fwdstop_minute                  = ${dfi_minute_fw}       
...
#
# create a namelist for WRF for 1-hour intergration only
#
 echo " NAMELIST: creating namelist for real run in 1hour"
 rm -f namelist.input_dfi* 
 cat > ${DIR_RUN}/namelist.input_dfi_${mcphysics} << EOF3
 &time_control
 run_days                            = 0,
 run_hours                           = 0,
 run_minutes                         = 0,
...
if [ -f $DIR_RUN/namelist.input_dfi_${mcphysics} ]; then
    ln -sf $DIR_RUN/namelist.input_dfi_${mcphysics} ./namelist.input_dfi
